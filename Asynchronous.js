...
/*
Our PC sent request to API then API will send the response in XML/JSON. 
API has Basic URL and multiple endpoints
JSON:-JAVSCRIPT OBJECT NOTATION

json():-parsing JSON into JS object
fetch():-is used for sending/receiving Data to API. 
*/
let URL="https://emojihub.yurace.pro/api/random"
const getFact=async ()=>{
    console.log("getting Data")
    let response=await fetch(URL)
    console.log(response,"is first Promise Object(JSON)")
    //The process of sending and receiving data to API is called AJAX(Asynchronous JS and XML/JSON)
    let data=await response.json()//second promise Object(JS object)
